namespace gdjs {
  ...
  /**
   * The renderer for a gdjs.RuntimeGame using Three.js as the primary renderer.
   * @category Renderers > Game
   */
  export class RuntimeGameRendererImpl {
    ...
    initializeRenderers(gameCanvas: HTMLCanvasElement): void {
      ...
      if (typeof THREE === 'undefined') {
        throw new Error(
          'Three.js must be loaded before initializing the 3D-first game renderer.'
        );
      }
      ...
      if (!gl) {
        throw new Error(
          'Unable to create a WebGL2 context. The 3D-first renderer requires WebGL2.'
        );
      }
      ...
      try {
        ...
        logger.info('WebGL2 context created. Three.js renderer enabled.');
        ...
        logger.error('Three.js WebGLRenderer initialization failed.', error);
    ...
    initializeCanvas(gameCanvas: HTMLCanvasElement): void {
      ...
      // Pixel rounding is handled by object renderers in the Three-first runtime.
    ...
    private _resizeCanvas() {
      if (!this._threeRenderer || !this._domElementsContainer) return;

      // Set the Three renderer size to the game size.
      // There is no "smart" resizing to be done here: the rendering of the game
      // should be done with the size set on the game.
      if (
        this._threeRenderer.domElement.width !==
          this._game.getGameResolutionWidth() ||
        this._threeRenderer.domElement.height !==
          this._game.getGameResolutionHeight()
      ) {
        this._threeRenderer.setSize(
          this._game.getGameResolutionWidth(),
    ...
    /**
     * Dispose the Three.js renderer as well as DOM elements used for the game
     * (the canvas, if specified, and the additional DOM container created on top
     * of it to allow display HTML elements, for example for text inputs).
     *
  ...
  /** @category Renderers > Game */
  export type RuntimeGameRenderer = RuntimeGameRendererImpl;
  /** @category Renderers > Game */
  export const RuntimeGameRenderer = RuntimeGameRendererImpl;
}
